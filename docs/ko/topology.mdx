---
title: 토폴로지 및 워크시트
description: GRE 터널 구성을 위한 네트워크 토폴로지 다이어그램 및 IP 주소 지정 워크시트.
sidebar:
  order: 3
i18n:
...
클라우드 스크러빙 센터에 연결하는 **xDC_NAMEx** 데이터 센터에 대한 구성입니다.

:::note
**이것은 예시 값입니다.** 위의 표를 사용하여 고객별 및 SOC 제공 값으로 대체하십시오.

보호된 프리픽스는 **공개적으로 라우팅 가능해야 합니다** (RFC 1918 비해당).
터널이 공용 인터넷을 통과할 때 GRE 외부 엔드포인트 IP도 공개적으로 라우팅 가능해야 합니다. 사설 연결(L2, 사설 피어링)은 RFC 1918 엔드포인트를 허용할 수 있습니다. 적절한 문서 주소를 사용하는 예시는
[K000147949](https://my.f5.com/manage/s/article/K000147949)를 참조하십시오.

이중화를 위해 서로 다른 지리적 위치의 스크러빙 센터로 **BIG-IP 유닛당 2개의 터널**을 생성하십시오 (HA 쌍의 경우 총 4개의 터널).
:::
...
터널 구성을 구축할 때 다음 XC 및 BIG-IP 워크시트를 참고로 사용하십시오.
...
GRE 터널 내부에 캡슐화되어 공용 인터넷에 노출되지 않으므로 올바른 방식입니다. 보호된 프리픽스는 항상 공개적으로 라우팅 가능해야 하며, 터널이 공용 인터넷을 통과할 때 외부 엔드포인트 IP도 공개적으로 라우팅 가능해야 합니다.
:::

:::note[IPv6 내부 링크]
IPv6 내부 링크는 일반적인 클라우드 기본값에 맞추기 위해 여기서 /64 프리픽스를 사용합니다. 포인트-투-포인트 링크의 경우 neighbor-discovery 고갈을 방지하기 위해
[RFC 6164](https://datatracker.ietf.org/doc/html/rfc6164)에 따라 /127이 권장됩니다. SOC 터널 할당이 지원하는 경우 /127을 사용하십시오.
:::
