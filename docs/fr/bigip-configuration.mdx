...
description: >-
  Commandes tmsh et imish pour configurer les tunnels GRE, les Self IPs et le
  BGP sur les paires HA BIG-IP.
sidebar:
  order: 5
i18n:
...
Toutes les commandes ci-dessous sont exécutées dans **tmsh** sur BIG-IP. Ajustez les noms des objets
et les adresses IP selon vos besoins.

Pour la configuration générale des tunnels GRE sur BIG-IP, consultez
[Configuring a GRE Tunnel Using BIG-IP](https://community.f5.com/kb/technicalarticles/configuring-a-generic-routing-encapsulation-gre-tunnel-using-big-ip/289030). Pour la configuration
initiale du routage avec le Cloud, consultez [K000147949](https://my.f5.com/manage/s/article/K000147949).
...
Chaque tunnel relie une unité BIG-IP à un point de terminaison d'un centre de nettoyage Cloud. Créez **deux tunnels par unité** (un vers
chaque centre de nettoyage géolocalisé) pour un total de **quatre tunnels
logiques** sur l'ensemble de la paire HA :
...
seront fragmentés ou rejetés. Définissez le MTU du tunnel pour prendre en compte
...
Ajustez si le Path MTU de votre liaison amont est inférieur. Assurez-vous que les messages ICMP
« destination unreachable / fragmentation needed » (type 3, code 4)
ne sont **pas** bloqués sur les équipements intermédiaires afin que la découverte du Path MTU
(PMTUD) fonctionne correctement.
:::

### Anti-usurpation GRE (ACLs en amont)

GRE (protocole IP 47) ne fournit aucune authentification. Quiconque
...
s'applique aux ACLs IPv6 pour les tunnels GRE IPv6. Les adresses IP externes
de BIG-IP-A (`xBIGIP_A_OUTER_V4x`) et de BIG-IP-B (`xBIGIP_B_OUTER_V4x`)
doivent être autorisées comme destinations GRE.
:::

### Self IPs internes (appairage BGP)

Assignez des adresses IP internes (à l'intérieur du tunnel GRE) qui formeront la
**session BGP** avec le Cloud. Le paramètre `allow-service` doit
inclure `tcp:179` (BGP) pour que la session d'appairage puisse s'établir. L'ajout de
`icmp:any` sur les Self IPs internes active le PMTUD et les tests d'accessibilité
à travers le tunnel :
...
unité ne configure que les voisins pour ses propres tunnels :

- **BIG-IP-A** configure les voisins C1-T1 et C2-T1.
- **BIG-IP-B** configure les voisins C1-T2 et C2-T2.

Le `router-id` doit être unique par unité (utilisez la Self IP externe
de chaque unité).
:::
...
1. Entrez en mode privilégié puis en mode configuration :
...
:::caution[Remplacez les valeurs substituables]
- `xCUSTOMER_ASNx` — votre ASN public enregistré auprès de l'ARIN (ou
  ...
  Un ASN privé (64512-65534 / 4200000000-4294967294) n'est acceptable que
  si le SOC prend explicitement en charge l'appairage avec ASN privé pour
  votre déploiement.
...
- **`timers 10 30`** — Keepalive toutes les 10 s, temps de maintien 30 s. Les
  valeurs par défaut (60 / 180) sont trop lentes pour le basculement en cas de mitigation DDoS.
  Coordonnez les valeurs des minuteurs avec le SOC pour assurer la correspondance des deux côtés.
- **`ttl-security hops 1`** — Active GTSM
  ...
  via le tunnel GRE, cela empêche l'usurpation BGP distante en
...
- **`redistribute kernel`** — Injecte les routes statiques `null0` dans
  ...
  route-map stricte est présenté ici pour sa flexibilité.
...
Le BFD offre une convergence plus rapide que les minuteurs keepalive BGP seuls.
...
le routage normal en conditions normales (hors attaque). Si le préfixe protégé
