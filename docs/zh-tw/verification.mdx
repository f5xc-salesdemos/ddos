---
title: 驗證
description: 驗證 BIG-IP 上的 GRE 隧道與 BGP 配置，並排除常見問題。
sidebar:
  order: 6
i18n:
...
1. 確認 GRE 隧道已啟用：`show net tunnels tunnel` 應顯示狀態為 `up`。
1. 確認內部自身 IP 的 `allow-service` 包含 `tcp:179`。
1. 透過隧道對遠端內部 IP 執行 Ping 測試。
1. 確認兩端的 BGP 密碼一致（這是造成 `NOTIFICATION` 錯誤的常見原因）。
1. 確認前綴列表與路由映射已正確套用。
1. 檢查 `show ip bgp neighbors <IP>` 以查看最後的錯誤或重設原因。
1. 在每個 HA 單元上，確認該單元自身的隧道已啟用，且已配置其自身的 BGP 鄰居（而非另一單元的）。
:::
