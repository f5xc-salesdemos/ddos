---
title: 拓扑与工作表
description: GRE 隧道配置的网络拓扑图和 IP 地址规划工作表。
sidebar:
  order: 3
i18n:
...
**xDC_NAMEx** 数据中心连接至云清洗中心的配置。

:::note
**以下均为示例值。** 请使用上方表格中客户专属值及
SOC 提供的值进行替换。

受保护前缀**必须为公网可路由地址**（非 RFC 1918）。
当隧道穿越公共互联网时，GRE 外层端点 IP 也必须为公网可路由地址；私有连接（L2、私有对等互联）可允许使用 RFC 1918 端点。有关使用正确文档地址的示例，请参阅
[K000147949](https://my.f5.com/manage/s/article/K000147949)。

为确保冗余，请为每台 BIG-IP 单元**创建 2 条隧道**，分别连接至不同地理位置的清洗中心（HA 对共 4 条隧道）。
:::
...
:::note[内层（传输）IP]
内层 IP（如 `10.10.10.0/30`）使用 RFC 1918 地址，这是
正确的，因为它们被封装在 GRE 隧道内部，不会出现在公共互联网上。受保护前缀必须始终为公网可路由地址；当隧道穿越公共互联网时，外层端点 IP 也必须为公网可路由地址。
:::

:::note[IPv6 内层链路]
此处 IPv6 内层链路使用 /64 前缀，以匹配常见的
云端默认配置。对于点对点链路，建议按照
[RFC 6164](https://datatracker.ietf.org/doc/html/rfc6164) 使用 /127，以避免邻居发现耗尽问题。如果 SOC 隧道分配支持，请使用 /127。
:::

### BIG-IP

**BIG-IP-A**（外层 IP `xBIGIP_A_OUTER_V4x` / `xBIGIP_A_OUTER_V6x`）：
...
**BIG-IP-B**（外层 IP `xBIGIP_B_OUTER_V4x` / `xBIGIP_B_OUTER_V6x`）：
