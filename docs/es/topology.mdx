...
:::note
**Estos son valores de ejemplo.** Reemplácelos con los valores específicos del cliente y
los proporcionados por el SOC utilizando las tablas anteriores.

Los prefijos protegidos **deben ser enrutables públicamente** (no RFC 1918).
Las IPs de los endpoints externos GRE también deben ser enrutables públicamente cuando los túneles
atraviesan la Internet pública; la conectividad privada (L2, peering
privado) puede permitir endpoints RFC 1918. Consulte
[K000147949](https://my.f5.com/manage/s/article/K000147949) para ejemplos usando direcciones
de documentación apropiadas.

Para redundancia, cree **2 túneles por unidad BIG-IP** hacia diferentes
centros de depuración geo-localizados (4 túneles en total para un par HA).
:::
...
si la asignación de túnel del SOC lo admite.
:::
